    ...
    public function stoteQuestions()
    {
      if(Input::hasFile('import_file')){
        ...
        if(!empty($data) && $data->count()){
          ...
          foreach ($data as $key=>$result) {
            if(isset($result["question_text"]) && isset($result["question_type"]) && isset($result["question_required"]) && isset($result["question_enabled"]) && isset($result["question_dimension"]) && isset($result["display_order"]) && isset($result["options"])
            && isset($result["option_weight"])){
              ...
              elseif ($result->question_enabled=="") {
                ...
                elseif (($result->question_type=="welcome_text") && (count($welcome_count)!="0")) {
                  $error[]="Welcome text Already exists line@ $i";
                }
                elseif (($result->question_type=="thankyou_text") && (count($thank_count)!="0")) {
                  $error[]="Thank you text Already exists line@ $i";
                }
                ...
                elseif ($result->question_type=='welcome_text' && $result->options!= 'Null' || $result->question_type=='welcome_text' && $result->option_weight!= '0' ) {
                  $error[]="You are question_type select in 'welcome_text' Please enter option  to null and option weight enter to 0. line@ $i";
                }
      ...
      if(isset($error))
      {
        ...
        foreach ($data as $key=>$result) {
          ...
          $lastinsert_id=DB::table('questions')->insertGetId(['survey_id'=>'2','question_text'=>$result->question_text,'question_type'=>$result->question_type,'question_required'=>$required_enable,'question_enabled'=>$required_enable,'question_dimension'=>$result->question_dimension,'display_order'=>$result->display_order]);
